...
public class InviteIntegrationTests : IDisposable
{
    ...
    [Fact]
    public async Task GuestIsInvited_ShouldHavePendingStatusInEventInvitation()
    {
        // Arrange
        ...
        // Act
        var result = await _commandDispatcher.DispatchAsync(commandInvite);

        // Assert
        Assert.True(result.isSuccess);
        Assert.Single((await repoEvent.GetAsync(commandCreate.VeaEvent.VeaEventId.Id))._invitations);
        Assert.Equal(StatusType.Pending,
