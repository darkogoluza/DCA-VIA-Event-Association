...
public class DeclineInvitationIntegrationTest : IDisposable
{
    ...
    [Fact]
    public async Task GivenGuestIsInvited_WhenTheyDecline_ThenInvitationIsMarkedAsRejected()
    {
        // Arrange
        ...
        var commandDeclineInvite = GuestDeclineInvitationCommand
            .Create(commandCreate.VeaEvent.VeaEventId.Id, commandRegister.Guest.GuestId.Id).payload;

        // Act
        var result = await _commandDispatcher.DispatchAsync(commandDeclineInvite);

        // Assert
        Assert.True(result.isSuccess);
        Assert.Single((await repoEvent.GetAsync(commandCreate.VeaEvent.VeaEventId.Id))._invitations);
        Assert.Equal(StatusType.Rejected,
