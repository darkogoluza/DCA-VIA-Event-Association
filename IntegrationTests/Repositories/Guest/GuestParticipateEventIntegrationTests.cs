...
public class GuestParticipateEventIntegrationTests : IDisposable
{
    ...
    [Fact]
    public async Task GuestParticipation_ShouldSucceed_WhenEventIsProperlyConfiguredAndActiveTest()
    {
        // Arrange
        ...
        // Act
        var result = await _commandDispatcher.DispatchAsync(commandParticipateEvent);

        // Assert 
        Assert.True(result.isSuccess);
        Assert.Single((await repoEvent.GetAsync(commandCreate.VeaEvent.VeaEventId.Id))._guests);
    }
}