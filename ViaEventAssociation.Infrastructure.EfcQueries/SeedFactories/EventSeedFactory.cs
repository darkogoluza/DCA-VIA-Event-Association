...
public class EventSeedFactory
{
    ...
    {
        List<TmpEvent> eventTmps = JsonSerializer.Deserialize<List<TmpEvent>>(EventsAsJson)!;

        List<Event> events = eventTmps.Select(evt => new Event
        {
            Id = evt.Id,
            Title = evt.Title,
            Description = evt.Description,
            EventStatusType = evt.Status,
            Visibility = evt.Visibility == "public" ? 1 : 0,
            StartDateTime = evt.Start,
            EndDateTime = evt.End,
            MaxNoOfGuests = evt.MaxGuests,
        }).ToList();
