...
{
    private const string ParticipationsAsJson = """
                                                [
                                                  {
    ...
    public static void AddParticipationsToEvent(VeadatabaseProductionContext context)
    {
        List<TempParticipation> tempParticipations =
            JsonSerializer.Deserialize<List<TempParticipation>>(ParticipationsAsJson)!;

        foreach (var tempParticipation in tempParticipations)
        {
            var guest = context.Guests.Single(g => g.Id == tempParticipation.GuestId);
            context.Events.Single(e => e.Id == tempParticipation.EventId).Guests.Add(guest);
        }
    }
}

public record TempParticipation(string EventId, string GuestId);
