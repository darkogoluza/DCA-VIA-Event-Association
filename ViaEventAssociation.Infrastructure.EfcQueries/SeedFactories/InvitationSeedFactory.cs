...
public class InvitationSeedFactory
{
    ...
    private static void AddInvitationsToEvent(VeadatabaseProductionContext context, string eventId)
    {
        List<TempInvitation> tempInvitations = JsonSerializer.Deserialize<List<TempInvitation>>(InvitationsAsJson)!;

        IEnumerable<Invitation> invitations = tempInvitations
            .Where(i => i.EventId == eventId)
            .Select(invt => new Invitation
            {
                Id = Guid.NewGuid().ToString(),
                InviteeId = invt.GuestId,
                EventId = invt.EventId,
                StatusType = invt.Status
            });
    ...
    public static void Seed(VeadatabaseProductionContext context)
    {
        ...
        {
            AddInvitationsToEvent(context, veaEvent.Id);
        }
    }
}

public record TempInvitation(string EventId, string GuestId, string Status);
